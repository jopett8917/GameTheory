...
def get_action(player, history, opponent_history, score, opponent_score, getting_team_name=False):
    ...
    #
        ...
        if getting_team_name:
            return 'backstab every 4th round-Kylie Frohlich'
        else:
            # use history, opponent_history, score, opponent_score
            # to compute your strategy
            size = len(history)
            if(size%4==0): #the number of rounds played is a multiple of 3
                return 'b'
            else:
                return 'c'
 ...
 #
        ...
        if getting_team_name:
            return 'loyal vengeful-hunter newton'
        else:
            # use history, opponent_history, score, opponent_score
            # to compute your strategy
            if len(opponent_history)%10==0: #It's the even round: collude
                return 'c'
    ...
    ######       
 #
    # This example player is random-Doug Stobie
    if player == 8:
        if getting_team_name:
            return 'stobie'
        else:    
    ...
    #
        ...
        if getting_team_name:
            return 'vengeful loyal occasionally greedy-Melodee Strauch'
        else:
    ...
    #
        ...
        if getting_team_name:
            return 'Give them a chance-Ryan and Tanner'
        else:
            # use history, opponent_history, score, opponent_score
            # to compute your strategy
            mean = False
            if len(opponent_history)==1 or 2 or 3: #It's the first round: collude
                return 'c'
            elif opponent_history[-1 or -2 or -3]=='b':
                mean=True
            if mean==True:
                return 'b'
            if mean==False:
   ...
   #
        ...
        if getting_team_name:
            ...
            # to compute your strategy
            size = len(history)
            if(size%5==0): # if round number is a multiple of 5
                return 'c'
            else :
                return 'b' 
    ...
    if player == 12:
        if getting_team_name:
            ...
            if len(opponent_history)==0: #It's the first round: betray
                ...
                # no match found
                if history[-1]=='c' and opponent_history[-1]=='b':
                    return 'c' # collude is they were severely punished last time
                else:
                    return 'b' #otherwise betray
    ...
    #
        ...
        if getting_team_name:
            return 'unexpected-PeterHolman'
        else:
            if len(opponent_history)==0: #It's the first round: collude
                    return 'b'
            if opponent_history=='b': #If the opponent colludes
                    return 'c'
                
            elif opponent_history[-1]=='c':
                    return 'b'
            elif history[-1]=='c' and opponent_history[-1]=='b':
                return 'b' # betray if they colluded before
            else:
                return 'b' #otherwise betray
    ...
    #
        ...
        global betrayed
        global betrayed_count
        global trying_luck
        global next_move
        if getting_team_name:
            return 'Kincaid And Ed'
        else:
            ...
            if len(opponent_history)==0: #It's the first round: collude 
                ...
                next_move = 'q'
                return 'c'
            #elif opponent_history[0] == 'b': # If your opponent betrayed the first round, give them no more hope.
             #   return 'b'
            elif next_move != 'q':
                getnextmove = next_move
                next_move = 'q'
                return getnextmove
            elif betrayed==True: # If betrayed is true, betray
                if history[-1] == 'b' and opponent_history[-1] == 'c':
                ...
                elif trying_luck>0: #If you give them a second chance maybee you can end up with better scores.
                    ...
                    if betrayed_count < 3:
                        ...
                        return 'b'
            elif history[-1]=='c' and opponent_history[-1]=='b': # If your opponent betrayed AND you trusted, set Betrayed to be true
                betrayed = True
                betrayed_count += 1
                return 'b'
            else:
    ...
    if player == 15:
        if getting_team_name:
            return 'vengeful-Kylie'
        else:
            if len(opponent_history)==0:
                return 'c'
            else: 
                if 'b' in str(opponent_history):
                    return 'b'
                else:
                    return 'c'
    ...
    ######
     #
        ...
        if getting_team_name:
            ...
            # use history, opponent_history, score, opponent_score
            # to compute your strategy
            if len(opponent_history) % 3 == 0: #It's the even round: collude
                return 'c'
    ...
    #
        ...
        if getting_team_name:
            return 'Give them a chance-Ryan Sandau Tanner'
        else:
            x = random.randint(0,5)
            if len(opponent_history)==0:
                return 'c'
            elif opponent_history[-2]=='b' and x!=3:
                 return 'b'   
            elif opponent_history[-2]=='c' and x!=3:
                return 'c'
            elif x==3:
                return 'b'
    ...
    ######
        ...
        if getting_team_name:
            return 'backstab backstab Loyal - Clark'
        else:
            size = len(history)
            if(size%3==0):
                return 'c'
    ...
    ######
    ######    
    elif player == 19: #Copy code from here down
        if getting_team_name:
            return 'skynet-JacksonPetty'
        else:
            # set Nice/Mean
            NICE = False
